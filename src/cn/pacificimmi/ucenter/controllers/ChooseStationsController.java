...
public class ChooseStationsController extends Controller {
	public void index(){
		 ...
		 * 渲染视图
		 */
		this.renderJsp("/views/choosestations.jsp");
	}
...
//			this.addWhereSegmentByAnd("station.com_id=orgnization.org_id");
			...
			if(this.getId()!=null){
				...
				this.addWhereSegmentByAnd("station.station_id not in (select station_id from console_user_station where user_id='"+id+"')");
			}
			//查询名称
	...
	public void save(){
		...
		if(!sp.getIds().isEmpty() && sp.getId()!=null){
			String id=sp.getId();
			String[] ids = sp.getIds().split(",");
			StringBuffer logDes  = new StringBuffer();
			User user = User.dao.findById(id);
			logDes.append("为").append(user.getUserName()).append("分配");
			for(String s:ids){
				Db.update("insert into console_user_station (user_id,station_id) values('"+id+"','"+s+"')");
				Station sta = Station.dao.findById(s);
				logDes.append(sta.getName()).append("、");
			}
			logDes.append("岗位");
			OperateLogger.log(this, "用户管理", "岗位分配", logDes.toString());
			rst.put("status", 0);
			rst.put("msg", "填加成功");
			this.renderJson(rst);		
		}
